...
$(document).swipeLeft(function(){
	...
	last.row = now.row;
	last.col = now.col;
	if (last.row==1 && last.col==1) { now.row = last.row; now.col = 2; pageMove(towards.left);}	
})
...
$(document).swipeRight(function(){
	...
	last.row = now.row;
	last.col = now.col;
	if (last.row>0 && last.row<maxPage && last.col==2) { now.row = last.row; now.col = 1; pageMove(towards.right);}	
})
